        ...
        for record in test_data:
            ...
            try:
                ...
                for pos in range(self.positions):
                    if pos < len(numbers):
                        total[pos] += 1
                        # 检查预测的前7个号码是否包含实际开奖号码
                        top7 = [pred[0] for pred in predictions[pos][:7]]
                        if numbers[pos] in top7:
                            wins[pos] += 1
            except:
                continue
...
class DeepSeekPredictor:
    ...
    def predict_position(token: str, history_data: List[Dict]) -> Optional[Dict]:
        ...
        try:
            ...
            payload = {
                'model': 'deepseek-chat',  # 或 'deepseek-chat-v2'
                'messages': [
                    {
                        'role': 'system',
                        'content': '你是一个专业的彩票数据分析专家，擅长分析历史开奖数据并预测下一期的开奖号码。请严格按照JSON格式输出结果。'
                    },
                    {
                        'role': 'user',
                        'content': prompt
                ...
                'max_tokens': 3000  # 增加token数量以确保完整响应
            }
            ...
            if 'choices' in result and len(result['choices']) > 0:
                ...
                parsed_result = DeepSeekPredictor._parse_response(content)
                if parsed_result is None:
                    print(f"DeepSeek响应解析失败，原始内容:\n{content[:1000]}")
            ...
            print(f"DeepSeek API响应JSON解析错误: {e}")
            return None
                                    ...
                                    for pred in pred_list:
                                        if isinstance(pred, dict):
                                            ...
                                            if num and 1 <= num <= 10:
                                                ...
                                                predictions[pos].append((int(num), float(score)))
                                # 确保每个位置有7个号码
                                if len(predictions[pos]) < 7:
                                    # 如果不足7个，补充剩余号码
                                    existing_nums = [p[0] for p in predictions[pos]]
                                    for num in range(1, 11):
                                        if num not in existing_nums and len(predictions[pos]) < 7:
                                            predictions[pos].append((num, 0.1))
                        except (ValueError, KeyError) as e:
                            print(f"解析位置{pos_str}时出错: {e}")
                            continue
                
                # 确保所有10个位置都有预测
                for pos in range(10):
                    if pos not in predictions:
                        predictions[pos] = [(i, 0.1) for i in range(1, 8)]
                
                if predictions:
                    return {
                        'predictions': predictions,
                        'confidence': result.get('overall_confidence', 50) / 100.0 if isinstance(result.get('overall_confidence'), (int, float)) else 0.5,
                        'analysis': result.get('analysis', '')
                    }
            
            print(f"未找到有效的JSON格式，响应内容:\n{content[:1000]}")
            return None
...
class LotteryQueryApp:
    ...
    def __init__(self, root):
        ...
        self.comprehensive_predictions = None  # 存储综合推荐结果
    ...
    def display_results(self, results: List[Dict], source: str):
        ...
        # 自动运行算法预测（如果数据足够且自动刷新开启）
        if len(results) >= 10:
            if self.auto_refresh_enabled:
                self.auto_predict_algorithm()
        ...
        if self.comprehensive_predictions:
            self.status_label.config(text=f"成功加载 {len(results)} 条记录（已自动预测）", fg='green')
        else:
            self.status_label.config(text=f"成功加载 {len(results)} 条记录", fg='green')
    
    def auto_predict_algorithm(self):
        """自动运行算法预测（不显示窗口）"""
        ...
        def predict_thread():
            try:
                ...
                if predictions:
                    # 存储综合推荐结果
                    self.comprehensive_predictions = predictions.get('comprehensive', {})
                    # 刷新显示
                    self.root.after(0, self.refresh_display_with_predictions)
            except Exception as e:
                pass  # 静默失败，不影响主流程
        
        threading.Thread(target=predict_thread, daemon=True).start()
    
    def refresh_display_with_predictions(self):
        """使用预测结果刷新显示"""
        if self.current_results:
            self.display_results(self.current_results, self.current_source)
        ...
        def deepseek_thread():
            try:
                ...
                if deepseek_result:
                    ...
                    self.root.after(0, lambda: self.status_label.config(text="DeepSeek预测完成", fg='green'))
                else:
                    error_msg = "DeepSeek预测失败，可能的原因：\n1. API Token无效或过期\n2. 网络连接问题\n3. API响应格式异常\n\n请检查控制台输出的详细错误信息"
                    self.root.after(0, lambda: messagebox.showerror("错误", error_msg))
                    self.root.after(0, lambda: self.status_label.config(text="DeepSeek预测失败，请查看控制台", fg='red'))
    ...
    def show_prediction_results(self, predictions: Dict):
        ...
        tk.Label(title_frame, text="定位胆预测结果（推荐前7个号码）", 
                font=(self.default_font, 14, 'bold'),
        ...
        for algo_key, algo_name, algo_data in algorithms:
            ...
            headers = ['位置', '推荐1', '推荐2', '推荐3', '推荐4', '推荐5', '推荐6', '推荐7', '胜率']
            col_widths = [80, 80, 80, 80, 80, 80, 80, 80, 80]
            ...
            for pos in range(10):
                ...
                for col_idx in range(7):
                    if col_idx < len(pos_predictions):
                        ...
                        tk.Label(row_frame, text=f"{num} ({score_pct:.1f}%)", 
                               ...
                               width=10).grid(row=0, column=col_idx+1, padx=1, pady=5)
                    else:
                        tk.Label(row_frame, text="--", font=(self.default_font, 9),
                               bg=row_frame.cget('bg'), width=10).grid(row=0, column=col_idx+1, padx=1, pady=5)
                ...
                if algo_key == 'deepseek':
                    ...
                    tk.Label(row_frame, text=f"{pos_confidence:.1f}%", font=(self.default_font, 9, 'bold'),
                            bg=row_frame.cget('bg'), fg=color, width=10).grid(row=0, column=8, padx=2, pady=5)
                elif algo_key == 'comprehensive':
